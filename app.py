...
# ===== CONFIG =====
DATA_FILE = "synthetic_grid_outage_50sec_5s.csv"  # <-- keep this name
TICK_SECONDS = 1.0   # refresh every 1s
WINDOW_SECONDS = 120 # show last 2 minutes in view
...
# Inject Damage (fixed implementation)
...
if st.sidebar.button("🔥 Inject"):
    ...
    if pd.isna(t0):
        ...
        # Unique, sorted timestamps as a Series with clean integer index
        times = df_all["timestamp"].drop_duplicates().sort_values().reset_index(drop=True)
        # Find position to start (nearest >= t0)
        ...
        st.sidebar.success(f"Injected on {hot_asset} for {len(apply_times)} slices starting at {times.iloc[pos0]}.")
